...
public abstract class Agencia {
	...
	public void ajustarLimites(ContaEspecial[] contasEspeciais) {
		System.out
				.println("===================================================================");
		System.out.println("Agencia: " + this.getNumero()
				+ " ajustando limites");
		for (int i = 0; i < contasEspeciais.length; i++) {
			ContaEspecial ce = contasEspeciais[i];
			if (ce != null) {
				if (ce.getAgencia() != this) {
					System.out.println("A conta " + ce.getNumero()
							+ " nao pertence a esta agencia");
				} else {
	...
	public Conta abrirConta(Cliente titular, double saldoInicial) {
		...
		if (saldoInicial <= 500) {
			...
			return new ContaPoupanca(saldoInicial, numeroConta, titular, this,
					hoje);
		} else {
			double limite = 0.4 * saldoInicial;
			return new ContaEspecial(saldoInicial, numeroConta, titular, this,
					limite);
		}
	}
}