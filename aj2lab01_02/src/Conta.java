...
class Conta {
	...
	void saque(double valor) {
		if (valor > 0) {
			if (saldo > valor) {
				...
				System.out.println("Seu Saldo Atual eh de: R$" + saldo);
				System.out.println("\n----------------------------");
			...
			System.out.println("O valor informado Ž NEGATIVO");
		}
	}
	...
	void deposito(double valor) {
		...
		if (valor > 0) {
			...
			System.out.println("Seu saldo agora eh: R$" + saldo);
		} else {
			System.out.println("O VALOR INFORMADO NEGATIVO");

		}
		// valor > 0 ? saldo=saldo+valor : System.out.println("Valor ERRADO");
	}
